...
var speed = 1;
var firstName=""
//ввод имени в программу
function check_name()
{
    firstName = prompt('Как Вас зовут?');
    (Boolean(firstName))?alert("Добро пожаловать " + firstName):check_name()
}
...
function init() {
    ...
    if (canvas.getContext) {
        ...
        character = [];
        for (var i = 1; i <= 5; i++) {
            var item = new TBall(0 + Math.random() * (canvas.width - 30),
                0 + Math.random() * (canvas.height - 30));
            item.draw(ctx);
            character.push(item);
        }
    }
